...
const AverageGoals = () => {
    ...
    const [minGoalsFor, setMinGoalsFor] = useState<number>(0); // User input for average goals
    ...
    return (
        <div className="bg-white p-6 rounded shadow-md">
            ...
            <div className="mt-4 flex gap-4">
                ...
                <label className="block w-1/2">
                    <span className="text-gray-700">Min Avg Goals For:</span>
                    <input
                        type="number"
                        value={minGoalsFor}
                        onChange={(e) => setMinGoalsFor(parseInt(e.target.value))}
                        className="mt-1 block w-full px-3 py-2 border rounded shadow focus:ring focus:ring-blue-500"
                        placeholder="Enter min average goals"
                    />
                </label>
            </div>
                ...
                <table className="min-w-full border-collapse border border-gray-300 mt-4">
                    <thead>
                        <tr className="bg-gray-200">
                            ...
                            <th className="border border-gray-300 px-4 py-2">Goals For</th>
                            <th className="border border-gray-300 px-4 py-2">Goals Against</th>
                            <th className="border border-gray-300 px-4 py-2">Points</th>
                            <th className="border border-gray-300 px-4 py-2">Year</th>
                            <th className="border border-gray-300 px-4 py-2">Avg Goals For</th>
                        </tr>
                    </thead>
                    <tbody>
                        {records.map((record, index) => (
                            <tr key={index} className="text-center">
                                ...
                                <td className="border border-gray-300 px-4 py-2">{record.draw}</td>
                                <td className="border border-gray-300 px-4 py-2">{record.loss}</td>
                                <td className="border border-gray-300 px-4 py-2">{record.goals_for}</td>
                                <td className="border border-gray-300 px-4 py-2">{record.goals_against}</td>
                                ...
                                <td className="border border-gray-300 px-4 py-2">{record.avg_goals_for.toFixed(2)}</td>
                            </tr>
                        ))}
                    </tbody>
                </table>
